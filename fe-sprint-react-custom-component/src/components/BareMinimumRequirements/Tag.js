...
export const Tag = () => {
  ...
  const addTags = (event) => {
    ...
    // for (let tag of tags) { if (event.target.value !== tag && event.target.value.length > 0) {
    //     return setTags([...tags, event.target.value]);}}
    // for (let i = 0; i < tags.length; i++) {if (tags.includes(event.target.value) === -1 && event.target.value.length > 0) {
    //     return setTags([...tags, event.target.value]);}}
    //레퍼런스
    // if(event.key ==='Enter') {
    //   if(tags.includes(event.target.value) || event.target.value === "") {return null;}
    //   else setTags([...tags, event.target.value]);
    //   event.target.value = "";
    // }
          ...
          {tags.map((tag, index) => (
            <li key={index} className="tag">
              ...
              <span className="tag-close-icon" onClick={() => { removeTags(index) }}>
                {/* index를 넘겨주기위해 인자를 넣으려면 익명함수 형태로 기입한다 */}
                &#x2716;
